...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    console.log(`Fetching status for book ID: ${bookId}`);
    
    // Forward the request to the AI service
    const aiResponse = await fetch(`${AI_SERVICE_URL}/api/books/${bookId}/status`);
    ...
    console.log(`Successfully fetched status for book ID: ${bookId}`);
    return res.status(200).json(data);
  } catch (error: unknown) {
    console.error(`Error fetching status for book ID ${bookId}:`, error);
